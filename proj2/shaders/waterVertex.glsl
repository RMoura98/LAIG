...
void main() {
    ...
    vec2 NewaTextureCoord = vec2((aTextureCoord.s + time) * textScale, (aTextureCoord.t + time) * textScale);
    vec4 color = texture2D(uSampler2, NewaTextureCoord);
    
    vec4 NewaVertexPosition = vec4(aVertexPosition,1.0);

    NewaVertexPosition.y +=  color.r * color.a * heightScale;

    vec4 vertex = uMVMatrix * NewaVertexPosition;
